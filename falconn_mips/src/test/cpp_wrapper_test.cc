...
TEST(WrapperTest, DenseLshParaProber) {
  // typedef DenseVector<float> Point;
  int dim = 4;
...
TEST(WrapperTest, DenseLshParaProberAccuracy) {
  // typedef DenseVector<float> Point;
  int dim = 4;
  ...
  DenseLshParaProber<float> dlt(dim, 1000);
  dlt.set_print_file();
  double accuracy = dlt.evaluate_mips_accuracy(params);
...
TEST(WrapperTest, DenseLshParaProberAccuracyEvaluate) {
  // typedef DenseVector<float> Point;
  int dim = 4;
  ...
  DenseLshParaProber<float> dlt(dim, 10000, 200);
  dlt.set_print_file();
  dlt.print_para();
